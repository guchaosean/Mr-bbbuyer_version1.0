...
class WC_Swatch_Term {
    ...
    public function get_output($placeholder = true, $placeholder_src = 'default') {
        ...
        if ($this->type == 'photo' || $this->type == 'image') {
            ...
            $picker .= '<a href="' . $href . '" style="width:32px;height:32px;" title="' . $this->term_label . '" class="' . $anchor_class . '">';
        ...
        $picker .= '<a href="' . $href . '" style="text-indent:-9999px;width:32px;height:32px;background-color:' . apply_filters('woocommerce_swatches_get_swatch_color', $this->color, $this->term_slug, $this->taxonomy_slug, $this) . ';" title="' . $this->term_label . '" class="' . $anchor_class . '">' . $this->term_label . '</a>';
		} elseif ($placeholder) {
