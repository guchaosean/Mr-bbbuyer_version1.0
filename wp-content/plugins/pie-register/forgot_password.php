...
function pieResetFormOutput($piereg_widget = false){
	...
	*/
	...
	$warning 	= '<strong>'.ucwords(__("注意：","piereg")).'</strong>: '.__("请输入您的用户名/登录名或者邮箱，您将通过邮箱收到重置密码的链接。",'piereg');
	$success	= "";
	if(isset($_POST['user_login']) and trim($_POST['user_login']) == ""){
		$error[] = '<strong>'.ucwords(__("error","piereg")).'</strong>: '.__('无效的用户名或邮箱，请重新输入','piereg');
	}elseif (isset($_POST['reset_pass']))
	{
		...
		if($option['capthca_in_forgot_pass'] == 1){
			...
			if($privatekey){
				...
				if (!$resp->is_valid) {
					$error[] = '<strong>'.ucwords(__("error","piereg")).'</strong>: '.__('验证码错误','piereg');
					$error_found++;
				}
			}
			...
			*/
			...
			if ($user_exists){
			   ...
			   $message .= network_site_url("wp-login.php?action=rp&key=$key&login=" . rawurlencode($user_login), 'login') . "&redirect_to=".urlencode(get_option('siteurl'))."\r\n";*/
				...
				if (count($error) == 0 )
				{
					$success =  '<strong>'.ucwords(__("success","piereg")).'</strong>: '.apply_filters("piereg_message_will_be_sent_to_your_email",__('链接已经送达邮箱，请查询邮箱','piereg'));
				}	
			}
		}
    ...
    $forgot_pass_form .= '<input type="text" size="20" value="" class="input validate[required]" id="user_login" name="user_login" style="margin : 10px 0px;" placeholder="'.((isset($option['forgot_pass_username_placeholder']) && !empty($option['forgot_pass_username_placeholder']))? $option['forgot_pass_username_placeholder']: "").'">
		...
		$forgot_pass_form .= '<p class="submit">';
		  ...
		  $forgot_pass_form .= '
		  <input type="submit" value="'.__('重设密码',"piereg").'" class="button button-primary button-large" id="wp-submit" name="user-submit">
		</p>';
	...
	function forgot_pass_captcha($value = 0,$piereg_widget = false){
		...
		if($value == 1){//Re-Captcha
			...
			if($piereg_widget == true){
				...
				$data .= '<input id="" type="text" class="piereg_math_captcha_input" name="piereg_math_captcha_forgot_pass" required/>';				 $data .= '<label id="math_re_validation" type="text" style="width:auto;margin-top:2px; color:red; display:none;">验证码错误</label>';				 $data .='<script>				  $( document ).ready(function() {				      $( ".piereg_math_captcha_input" ).keyup(function() {						  if ($( ".piereg_math_captcha_input" ).val()!="'.$result.'"){$( "#math_re_validation" ).show(); $( ".piereg_math_captcha_input" ).css("border", "1px solid red");     $("#wp-submit").attr("disabled","disabled");}                          if ($( ".piereg_math_captcha_input" ).val()=="'.$result.'"){$( "#math_re_validation" ).hide(); $( ".piereg_math_captcha_input" ).css("border", "1px solid #c4c0bf"); $("#wp-submit").removeAttr("disabled");}				      });				   });								</script>';
