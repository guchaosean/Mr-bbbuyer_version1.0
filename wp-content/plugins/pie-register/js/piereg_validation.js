...
function ValidateField(rules,option,piereg_validate,IsWidget){
	...
	for (var i = 0; i < rules.length; i++) {
		switch(rules[i]){			case "mathcap":			    var url = "../wp-content/plugins/pie-register/js/check_mathcap.php";                    	             var checkstatus="";				 function ajaxCallBack(retString){                      checkstatus = retString;                 }                 $.ajax({                    type: "POST",                    url: url,                    data:'userinputmath='+ piereg(option).val().trim() ,  		                        success: function(data)                     {			               ajaxCallBack(data);					 					 },					  async: false		                  });			    if (checkstatus=="false")				{					ShowErrorMsg(option,getCustomFieldMessage(option,getAllRules.mathcapva_error.alertText));					breakLoop = true;				}else{                    					RemoveErrorMsg(option);				}						break;
			case "required":
			...
			case "custom":
				...
				switch(rules[i]){
					...
					break;					                    case "email":					   /* var url = "../wp-content/plugins/pie-register/js/check_email.php";                          	                    var checkstatus2="";				        function ajaxCallBack2(retString){                                checkstatus2 = retString;                        }                       $.ajax({                          type: "POST",                          url: url,                          data:'useremail='+ piereg(option).val().trim() ,  		                              success: function(data)                          {			                  ajaxCallBack2(data);						   					      },					      async: false                       });*/				       if(piereg(option).val().trim() != "")						{							console.log( rules[i] );							if(!piereg(option).val().trim().match(new RegExp(getAllRules[rules[i]].regex),piereg(option).val().trim()))							{								ShowErrorMsg(option,getCustomFieldMessage(option,getAllRules[rules[i]].alertText));								breakLoop = true;							}							/*else if (checkstatus2=="false")														{								ShowErrorMsg(option,getCustomFieldMessage(option,getAllRules[rules[i]].alertText2));								breakLoop = true;															}*/																					else{								RemoveErrorMsg(option);							}						}					break;				   				   				   				   
				}
			...
			break;*/			           /*-------------------------------------------------------My modified username for ajax user name call --------------------------*/						case "username":			     var url = "../wp-content/plugins/pie-register/js/check_username.php";                    	             var checkstatus="";				 function ajaxCallBack(retString){                      checkstatus = retString;                 }                 $.ajax({                    type: "POST",                    url: url,                    data:'username='+ piereg(option).val().trim() ,  		                        success: function(data)                     {			              ajaxCallBack(data);						  					 },					  async: false                });				 				if (checkstatus=="false")				{					ShowErrorMsg(option,getCustomFieldMessage(option,getAllRules.ajaxUserCall.alertText));					breakLoop = true;				}else{                    					RemoveErrorMsg(option);				}			break;
...
function getRegexAndErrorMsg(){
	var allRules ={
		...
		"email": {
			...
			"alertText": piereg_validation_engn[25], /*"* Invalid email address"*/			"alertText2": "改邮箱已用来被注册"
		},
		"integer": {
			"regex": /^[\-\+]?\d+$/,
		...
		},		"mathcapva_error":{									"alertText": "验证码错误"					}
	}
