...
$form_data = "";$form_data .=' <style>   @media screen and (max-width: 700px) {     .piereg_login_container{		 width:400px;         border-color: brown;         border-width: 2px;         border-style: dotted;		  background-color:white;	 }} @media screen and (max-width: 2000px) and (min-width: 701px){     .piereg_login_container{  background-color:white;  width:400px;  border-color: brown;  border-width: 2px;  border-style: dotted;   }	} </style>  ';
...
}else{    $form_data .='<style>h1{  color:white; !important;}	@media only screen and (min-width: 768px){		#rememberme{			 top: 2px;             position: relative;	    }		}h1 a { color:white; font-family: "Verdana" ;}#wrapper {    background: #000000; !important;}@media only screen and (min-width: 768px){#header #navigation ul.nav > li a {  border: 0px solid #000000; !important;  background: #000000; !important;}#navigation a{ color:white;  !important;} #navigation  li:hover a {   color:black;  !important;}}</style>			</style>';
	$form_data .= '
	<form method="post" action="" class="piereg_loginform" name="loginform" style="width:400px;">
		<div style="display:flex; width:400px;  padding: 20px;">';
		
	if(isset($option['login_username_label']) && !empty($option['login_username_label'])){
			$form_data .= '<label style="width:100px;"for="user_login">'.((isset($option['login_username_label']) && !empty($option['login_username_label']))? __($option['login_username_label'],"piereg") : __("Username","piereg")) .'</label>';
	}
	$user_name_val = ((isset($_POST['log']) && !empty($_POST['log']))?$_POST['log']:"");
	$form_data .= '<input style="width:250px;" placeholder="'.((isset($option['login_username_placeholder']) && !empty($option['login_username_placeholder']))? __($option['login_username_placeholder'],"piereg") : "").'" type="text" size="20" value="'.$user_name_val.'" class="input validate[required]" id="user_login" name="log">
		</div>
		<div style="display:flex; width:400px;  padding: 20px;">';
	
	if(isset($option['login_password_label']) && !empty($option['login_password_label'])){
		$form_data .= '<label style="width:100px;" for="user_pass">'.((isset($option['login_password_label']) && !empty($option['login_password_label']))? __($option['login_password_label'],"piereg") : __("Password","piereg")).'</label>';
	}
	
	$form_data .= '
			<input style="width:250px; padding: .236em .5em;    border: 1px solid #c4c0bf;    -webkit-border-radius: 0.125em;    border-radius: 0.125em;    -moz-background-clip: padding;    -webkit-background-clip: padding-box;    background-clip: padding-box;    font-size: 1em;    line-height: 1.618em;    font-family: inherit;    outline: none;    margin: 0 .327em;    background: #f7f7f7;    -webkit-box-sizing: border-box;    -moz-box-sizing: border-box;    box-sizing: border-box;    -webkit-appearance: none;    -webkit-box-shadow: 0 0 0 0.327em rgba(0, 0, 0, 0.03), inset 0 1px 2px #d8d4d3, 0 1px #ffffff;    box-shadow: 0 0 0 0.327em rgba(0, 0, 0, 0.03), inset 0 1px 2px #d8d4d3, 0 1px #ffffff;" placeholder="'.((isset($option['login_password_placeholder']) && !empty($option['login_password_placeholder']))? __($option['login_password_placeholder'],"piereg") : "").'" type="password" size="20" value="" class="input validate[required]" id="user_pass" name="pwd">
		</div>';
		...
		if($option['capthca_in_login'] != 0 && !empty($option['capthca_in_login'])){
			if($piereg_math_captcha_login == false && $piereg_widget == false){
				$form_data  .= '<div style="display:flex; width:400px;  padding: 20px;">';
				if(!empty($option['capthca_in_login_label']))
					$form_data  .= '<label style="margin-top:0px; width:100px;">'.$option['capthca_in_login_label'].'</label>';
		...
		/*if(!is_page()) {*/
			$form_data .= '            <div style="width:400px; display:flex;">
			<div class="forgetmenot" style="width:200px">
				<label style="left: 20px; position: relative;" for="rememberme">
					<input  type="checkbox" value="forever" id="rememberme" name="rememberme"> '.__("记住我","piereg").'
				</label>                  
			</div>';$form_data .= '<div id="nav" style=" width:200px;">';			if($users_can_register == 1){						$form_data .= '<a href="'.pie_registration_url().'">'.__("注册","piereg").'</a>&nbsp;<a style="cursor:default;text-decoration:none;" href="javascript:;">&nbsp;|&nbsp;</a>&nbsp;';			}			$form_data .= '<a title="'.__("Password Lost and Found","piereg").'" href="'.pie_lostpassword_url().'">'.__("忘记密码?","piereg").'</a> </div></div>';
		/*}*/        
		$form_data .= '
		<!--<div class="submit" >-->          <div>
			<input type="submit" value="'.__("登陆","piereg").'" class="button button-primary button-large" id="wp-submit" name="wp-submit" style="background-color: #777777;    background-image: -webkit-gradient(linear, left top, left bottom, from(#777777), to(#444444));	width:370px;    background-image: -webkit-linear-gradient(top, #777777, #444444);    background-image: -moz-linear-gradient(top, #777777, #444444);    background-image: -ms-linear-gradient(top, #777777, #444444);    background-image: -o-linear-gradient(top, #777777, #444444);    background-image: linear-gradient(to bottom, #777777, #444444);    text-shadow: 0 1px 0 #333333;    border: 1px solid #222222;    -webkit-box-shadow: 0 0 0 0.327em rgba(0, 0, 0, 0.075), 0 1px 2px rgba(0, 0, 0, 0.2), inset 0 1px #999999, inset 0 -1px #333333;    box-shadow: 0 0 0 0.327em rgba(0, 0, 0, 0.075), 0 1px 2px rgba(0, 0, 0, 0.2), inset 0 1px #999999, inset 0 -1px #333333;">
		...
		if($value == 1){//Re-Captcha
			...
			if($piereg_widget == true){
				...
				$field_id = "#pieregister_math_captha_login_form_widget";
								...
								$data .= '<div id="pieregister_math_captha_login_form" style=" padding:0px; top:-4px; position:relative;" class="piereg_math_captcha"></div>';
				$data .= '<input id="piereg_math_captcha_login" type="text" style="width:150px; top: -2px; position: relative; " name="piereg_math_captcha_login" required/>';				$data .= '<label id="math_re_validation" type="text" style="width:200px;margin-top:2px; color:red; display:none;">验证码错误</label>';                $data .='<script>				  $( document ).ready(function() {				      $( "#piereg_math_captcha_login" ).keyup(function() {						  if ($( "#piereg_math_captcha_login" ).val()!="'.$result.'"){$( "#math_re_validation" ).show(); $( "#piereg_math_captcha_login" ).css("border", "1px solid red");     $("#wp-submit").attr("disabled","disabled");}                          if ($( "#piereg_math_captcha_login" ).val()=="'.$result.'"){$( "#math_re_validation" ).hide(); $( "#piereg_math_captcha_login" ).css("border", "1px solid #c4c0bf"); $("#wp-submit").removeAttr("disabled");}				      });				   });								</script>';
