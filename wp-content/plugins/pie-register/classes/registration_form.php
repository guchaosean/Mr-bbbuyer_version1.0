...
class Registration_form extends PieReg_Base
{
	...
	{			
		...
		else if($this->field['date_type'] == "datedropdown")
		{
			...
			if($this->field['date_format']=="mm/dd/yy")
			{
				  ...
				  </div>				  <span style="left: 10px; position: relative;">（可选，临近生日时会获赠一份小礼物）</span>
				  <!--<div class="time_fields">
	...
	function addClass($default = "input_fields",$val = array())
	{
		...
		if ($this->field['type']=="math_captcha"){			$val[] = "mathcap";					}
	...
	{	
		...
		if($piereg_widget == true){
			...
			$data .= 'document.cookie="piereg_math_captcha_registration='.$result1."|".$result2."|".$result3.'";';            			$_SESSION["checkmathpa"] = $result;			
		}
		$data .= '</script>';
		...
		if($piereg_widget == true){
	      ...
	      $data .= '<input id="'.$this->id.'" type="text" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-underflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-overflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" class="'.$this->addClass().'" placeholder="'.$this->field['placeholder'].'" style="width:auto;margin-top:9px;" name="piereg_math_captcha"/>';          //$data .= '<input id="'.$this->id.'" type="text" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-underflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-overflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" class=	"input_fields  piereg_validate[required]" placeholder="'.$this->field['placeholder'].'" style="width:auto;margin-top:9px;" name="piereg_math_captcha"/>';
	...
	function addSubmit()
	{
		...
		$data .= '<div class="fieldset piereg_submit_button " style="text-align: center;">';
		if($this->pages > 1)
		{
		...
		{
			$data .= '<input style=" background-color: #777777;    background-image: -webkit-gradient(linear, left top, left bottom, from(#777777), to(#444444));    background-image: -webkit-linear-gradient(top, #777777, #444444);    background-image: -moz-linear-gradient(top, #777777, #444444);    background-image: -ms-linear-gradient(top, #777777, #444444);    background-image: -o-linear-gradient(top, #777777, #444444);    background-image: linear-gradient(to bottom, #777777, #444444);    text-shadow: 0 1px 0 #333333;    border: 1px solid #222222;    -webkit-box-shadow: 0 0 0 0.327em rgba(0, 0, 0, 0.075), 0 1px 2px rgba(0, 0, 0, 0.2), inset 0 1px #999999, inset 0 -1px #333333;    box-shadow: 0 0 0 0.327em rgba(0, 0, 0, 0.075), 0 1px 2px rgba(0, 0, 0, 0.2), inset 0 1px #999999, inset 0 -1px #333333;" name="pie_submit" type="submit" value="'.__($this->field['text'],"piereg").'" />';
		}
			...
			else if($field['type']=="math_captcha")
			{
				if(isset($_POST['piereg_math_captcha']))
				{
					...
					if( ($cookie_result1 == $cookie_result2) && ($cookie_result3 == $_POST['piereg_math_captcha'])){
						...
						$errors->add('math_captcha_mismatch',"<strong>".ucwords(__('error','piereg'))."</strong>: ". apply_filters("piereg_Invalid_Security_Code",__("验证码错误！", 'piereg')));
					}
				}
