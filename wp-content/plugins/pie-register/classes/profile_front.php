...
class Profile_front extends PieReg_Base
{
	...
	function print_user_profile()
    {
		...
		{
          	$data .= '<h1 id="piereg_pie_form_heading">'.__(" ","piereg").'</h1>';
			/*$data .= '<span class="piereg-profile-logout-url"><a href="'.wp_logout_url().'">'.__("登出","piereg").'</a></span>';*/
            /*$data .= '<a class="piereg_edit_profile_link" href="?page_id='.((isset($_GET['page_id']))?$_GET['page_id']:"").'&edit_user=1"></a>';*/            $data .='<a href="'.( add_query_arg( array("edit_user" => "1"), $this->piereg_get_current_url() ) ).'" style="  position: relative; top: -48px;float: right;">修改</a>';
			...
			if(is_array($this->data)){
			   ...
			   if(isset($this->field['show_in_profile']) && $this->field['show_in_profile']=="0")
			   {
				...
				switch($this->type) :				
					...
					case "default":
					$data .= '<tr><td class="fields fields2">'.$this->addLabel();
					$data .= '</td><td class="fields"><span>'.$this->getValue($this->type, $this->slug).'</span></td></tr>';										break;				    					case 'date':										$data .= '<tr><td class="fields fields2">'.$this->addLabel();					$date_value=$this->getValue($this->type, $this->slug);					$token = explode("/",$date_value);					$data .= '</td><td class="fields"><span>'.$token[0]."月".$token[1]."号".'</span></td></tr>';					//$data .= '</td><td class="fields"><span>'.$date_value.'</span></td></tr>';
					break;	
           ...
           $data .= '</table>';
	       //$data .= '<span class="piereg-profile-logout-url" style="background-color:#000000; float:right;"><a href="'.wp_logout_url().'" style="color:#ffffff; font-size:20px;">'.__("登出","piereg").'</a></span>';
        }
		return $data;
    }
	function getValue()
