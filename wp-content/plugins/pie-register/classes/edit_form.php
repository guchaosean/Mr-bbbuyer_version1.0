...
class Edit_form extends PieReg_Base
{
	...
	function addFormData()
	{
		...
		return '<h1 id="piereg_pie_form_heading">'.__(" ","piereg").'</h1>';
	}
	...
	function addPassword()
	{
		...
		$data .= '<label>'.__("旧密码 *","piereg").'</label><input id="old_password_'.$this->id.'" type="password" class="input_fields" value="" name="old_password" autocomplete="off"></div></li>';
	...
	function addSubmit()
	{
		...
		$data .= '<input type="button" style="background-color: #ff5911;"class="piereg_cancel_profile_edit_btn" onclick="location.replace(\''.($cancel_url).'\');" value="'.__("取消","piereg").'" />';
		/*$data .= '<input name="pie_submit_update" type="submit" value="'.__($this->field['text'],"piereg").'" />';	*/        $data .= '<input name="pie_submit_update" type="submit" value="'.__("更新","piereg").'" />';	
		return $data;
	}
