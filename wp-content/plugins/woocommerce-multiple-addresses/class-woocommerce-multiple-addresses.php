...
class WC_Multiple_addresses {
	...
	public function multiple_shipping_addresses() {
		...
		document.addEventListener('DOMContentLoaded', function() {
            var elements = document.getElementsByClassName('state_select');
  for (var i = 0; i < elements.length; i++) {
        elements[i].oninvalid = function(e) {
            e.target.setCustomValidity('');
            if (!e.target.validity.valid) {
                e.target.setCustomValidity('请在列表中选择地址所在地区');
            }
        };
		...
		if ( ! empty( $otherAddr ) ) {
			...
			foreach ( $otherAddr as $idx => $address ) {
				...
				echo '<p align="right"><a href="#" class="delete">' . __( '删除', self::$plugin_slug ) . '</a></p>';
				...
				if ( ! wc_ship_to_billing_address_only() && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) {
					...
					echo '<input type="checkbox" class="default_shipping_address" ' . $is_checked . ' value="' . $address['shipping_address_is_default'] . '"> ' . __( '设置为默认地址', self::$plugin_slug );
		  ...
		  // if (  get_option( 'woocommerce_calc_shipping' ) !== 'no' ) {
			echo '<input type="checkbox" class="default_shipping_address" checked value="true"> ' . __( '设置为默认地址', self::$plugin_slug );
		...
		echo '<div class="form-row">
                ...
                需要增加配送地址？请点<a class="add_address" href="#">' . __( '增加地址', self::$plugin_slug ) . '</a>
				 <p></p>
				<input type="submit" name="set_addresses" value="' . __( '保存地址', self::$plugin_slug ) . '" class="button alt" />
            </div>';
		echo '</form>';
		echo '</div>';
		?>
		<script type="text/javascript">
			var tmpl = '<div class="shipping_address address_block"><p align="right"><a href="#" class="delete"><?php _e( "删除", self::$plugin_slug ); ?></a></p>';
			...
			<?php if ( ! wc_ship_to_billing_address_only() && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) : ?>
				tmpl += '<input type="checkbox" class="default_shipping_address" value="false"> <?php _e( "设置为默认地址", self::$plugin_slug ); ?>';
	...
	public function save_multiple_shipping_addresses() {
		...
		if ( isset( $_POST['shipping_account_address_action'] ) && $_POST['shipping_account_address_action'] == 'save' ) {
			...
			if ( $is_default !== false ) {
				...
				endforeach;
				update_user_meta( $user->ID, 'wc_multiple_shipping_addresses_default', $default_address );
			}
			...
			if ( version_compare( WOOCOMMERCE_VERSION, '2.1', '<' ) ) {
				global $woocommerce;
				$woocommerce->add_message( __( '地址已保存', self::$plugin_slug ) );
			} else {
				wc_add_notice( __( '地址已保存', self::$plugin_slug ), $notice_type = 'success' );
			}
	...
	public function before_checkout_form() {
		...
		if ( is_user_logged_in() ) {
			/*echo '<p class="woocommerce-info woocommerce_message">
	                ' . __( '如果你有多个地址，在这可选择默认地址', self::$plugin_slug ) . '
	                <a class="button" href="' . get_permalink( $page_id ) . '">' . __( '设置地址', self::$plugin_slug ) . '</a>
	              </p>';*/
		}
	}
	...
	public function add_dd_to_checkout_fields( $fields ) {
		...
		$addresses    = array();
		$addresses[0] = __( '请选择配送地址（*若以下为正确地址或需填写新的订单地址，请忽略此项）', self::$plugin_slug );
		$china_count=0;
		$aus_count=0;
		for ( $i = 1; $i <= count( $otherAddrs ); ++$i ) {
           ...
           $addresses[ $i ] = $otherAddrs[ $i - 1 ]['shipping_first_name'] . ' ' . $otherAddrs[ $i - 1 ]['shipping_last_name'] . ', ' . $otherAddrs[ $i - 1 ]['shipping_address_1'] . ', ' . $otherAddrs[ $i - 1 ]['shipping_postcode'];
		   //if  ( $otherAddrs[ $i - 1 ]['shipping_country']=='AU'){ $aus_count=$aus_count+1; $addresses[ $i ] = "澳洲".$aus_count; }
		   //if  ( $otherAddrs[ $i - 1 ]['shipping_country']=='CN'){ $china_count=$china_count+1; $addresses[ $i ] = "中国".$china_count; }
		
		}

		$alt_field = array(
			'label'    => __( '已保存地址', self::$plugin_slug ),			 
			'required' => false,
