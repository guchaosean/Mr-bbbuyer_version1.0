...
if ( ! function_exists( 'woocommerce_button_proceed_to_checkout' ) ) {
	...
	function woocommerce_button_proceed_to_checkout() {
		$error_x= WC()->cart->check_cart_item_stock() ;
		  
		$checkout_url = WC()->cart->get_checkout_url();
		if ($error_x!="true"){
		 
		$code = $error_x->get_error_code();
        $messages = $error_x->get_error_messages($code);
		 
		}
	 if ( is_wp_error( $error_x ) ) {
...
function runEffect() {
   ...
   $( '#message' ).effect( 'shake', options, 500  );
   };
  </script> 
   ";
            echo '<div id="message" class="error" style="padding: 1.618em 1em .618em 2.618em;
  margin-bottom: 1.618em;
  background: #fff;
  border: 1px solid #4d65a4;
  border-left-width: .382em;
  position: relative;
  font-weight: bold;"><p>' . $error_string . '</p></div>';
			echo    "<a  href='javaScript:runEffect();' class='checkout-button button alt wc-forward'>". "前往收银台" ."</a>";
       }else{
		   
		    echo	"<a href=".$checkout_url." class='checkout-button button alt wc-forward'>"."前往收银台" ."</a>";
	   }
