...
class WC_Cart {
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function check_cart_item_stock() {
			global $wpdb;
            $check_error_or_not_all     =true;
			$check_error_or_not_single  =true; 
			$error               = new WP_Error();
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				...
				if ( ! $_product->is_in_stock() ) {
					...
					//return $error;
					$check_error_or_not_all=false;
					$check_error_or_not_single=false;
				}
				...
				if ($check_error_or_not_single==false){ $error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfill your order (%s in stock). Please edit your cart and try again. We apologise for any inconvenience caused.', 'woocommerce' ), $_product->get_title(), $_product->get_stock_quantity() ) );}
			     
			}

			  if ($check_error_or_not_all==true){return true;}
			  else{ return $error; }
		}
