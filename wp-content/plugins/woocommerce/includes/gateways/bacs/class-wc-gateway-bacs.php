	...
	public function __construct() {
		...
		$this->title        = $this->get_option( 'title' );
     	...
     	//$this->description  = $this->get_option( 'description' );
                         ...
                         $sql = "select * from wp_refee_id_log where mega_data='refee_number'";
                         $result = $conn->query($sql);

                         if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                $current_refeenumber= $row["value"] ;
                            
							}
                         }  
						 if ($current_refeenumber>=1000){ $current_refeenumber=1 ;}
						 if (strlen($current_refeenumber)==1){$show_number='00'.$current_refeenumber;}
						 if (strlen($current_refeenumber)==2){$show_number='0'.$current_refeenumber;}
						 if (strlen($current_refeenumber)==3){$show_number=$current_refeenumber;}
						 $future_refeenumber=$current_refeenumber+1;
						 $sql2 = "UPDATE wp_refee_id_log SET value='".$future_refeenumber."' WHERE mega_data='refee_number'";
						 if ($conn->query($sql2) === TRUE) {}						 
                         $conn->close();

		$this->description  = "<div>
		                             <input type='hidden' value='".$show_number."' name='bank_refee_number'> 
									  
									 
		                            
								<div style='display:flex;'>
									 <label  style='width:26%'> 请选择你的支付方式</label>
									 <select style='width:40%; font-size:14px;'  id='select_pay_way' name='select_pay_way' >
                                          <option value='au_pay'>澳洲银行转账</option>
                                          <option value='zhifubao'>支付宝(汇率*5)</option>
                                          <option value='china_pay'>国内银行转账(汇率*5)</option>                                         
                                     </select>
								</div>
		                           
									 
									 <div  style='  margin-left: 15px;  top:10px; position:relative;' id='au_pay_des'>
									  
									 <span  >我们的澳洲账户： 
                                                 Account Name: Mr BBBuyer 
                                                 BSB: 062010 
                                                 Account No.: 10502363 </span>
									 
									 </div>
									 <div   id='zhifubao_pay_des' style='display:none ;  margin-left: 15px; top:10px; position:relative;  '>
									  
									 <span  >我们的支付宝账户：
                                      min.tang.927@gmail.com
                                      收款人：唐旻</span>
									 
									 </div>
									 <div   id='china_pay_des'  style='display:none ;  margin-left: 15px;  top:10px; position:relative; ' >
									  
									 <span   >我们的国内银行账户：
银行：建设银行上海延长路支行
卡号：6222801215931016926
收款人：唐旻 </span>
									 ...
									 <span> 您的<b>Reference Number</b>为".$show_number."，转账时请使用此数字作为备注，我们将在资金3个工作日内到账后配送此订单。  </span>
...
<br>									 
									   <div style='display:flex;' >
				                           <label  style='width:60%;'> 上传转账截图（可选）</label>				    
					                       <input  style='width:80%;' type='file'   name='fileToUpload'>
										  
				                       </div>
									    <label style='top:10px; position:relative;'>确认转账成功，请点击下方“下单”键</label>
		                       </div>";
