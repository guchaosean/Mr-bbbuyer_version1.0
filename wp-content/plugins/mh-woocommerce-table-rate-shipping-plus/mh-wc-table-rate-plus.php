			...
			class MH_Table_Rate_Plus_Shipping_Method extends WC_Shipping_Method {
				...
				function update_licence() {
					if (isset($_POST[$this->id .'_old_licence_key'])) {
						$old_licence_key = wc_clean($_POST[$this->id .'_old_licence_key']);
				...
				*/
				...
				?>
					...
					<style>
						...
						td.mhtrsp-wide-table {
							padding-left: 0;
						}
					</style>
					<h2><?php _e('Table Rate Shipping Plus',MHTRP_DOMAIN); ?></h2>
					<?php
					$url = 'http://mangohour.com/support';
					$link = sprintf( wp_kses( __( 'For support and updates please visit the <a href="%s">Mangohour support website</a>.', MHTRP_DOMAIN ), array(  'a' => array( 'href' => array() ) ) ), esc_url( $url ) );
					?>
					<p><?php echo $link; ?></p>
				...
				function generate_zones_table_html() {
					...
					?>
					<tr valign="top">
						<td class="forminp mhtrsp-wide-table" id="<?php echo $this->id; ?>_zones" colspan="2">
							<div class="titledesc" style="font-weight: bold; color: #000;"><?php _e( 'Shipping Zones', MHTRP_DOMAIN ); ?></div>
							<p style="padding-bottom: 10px;"><?php _e( 'After adding a shipping zone, hit "Save changes" so that it appears as an option in the shipping rates section.', MHTRP_DOMAIN ); ?></p>
				...
				function generate_services_table_html() {
					...
					?>
					<tr valign="top">
						<td class="forminp mhtrsp-wide-table" id="<?php echo $this->id; ?>_services" colspan="2">
							<div class="titledesc" style="font-weight: bold; color: #000;"><?php _e( 'Shipping Services', MHTRP_DOMAIN ); ?></div>
							<p style="padding-bottom: 10px;"><?php _e( 'After adding a shipping service, hit "Save changes" so that it appears as an option in the shipping rates section.', MHTRP_DOMAIN ); ?></p>
							<table class="shippingrows widefat" cellspacing="0">
						        <col style="width:0%">
						        <col style="width:100%">
						         <col style="width:0%">
				...
				function generate_table_rates_table_html() {
					...
					?>
					<tr valign="top">
						<td class="forminp mhtrsp-wide-table" id="<?php echo $this->id; ?>_table_rates" colspan="2">
							<div class="titledesc" style="font-weight: bold; color: #000; padding-bottom: 10px;"><?php _e( 'Shipping Rates', MHTRP_DOMAIN ); ?></div>
							<table class="shippingrows widefat" cellspacing="0">
								<thead>
									<tr>
										<th class="check-column"><input type="checkbox"></th>
										...
										<th><?php _e( 'Base Cost', MHTRP_DOMAIN ); ?> <a class="tips" data-tip="<?php _e( 'Flat rate cost, excluding tax.', MHTRP_DOMAIN ); ?>">[?]</a></th>
						   	...
						   	<script type="text/javascript">
						   		...
						   		function copyLastRow(rowArr) {
						   			rowId = '#<?php echo $this->id; ?>_table_rates tbody tr:last';

						   			if (jQuery(rowId).length) {
						   				rowArr['zone'] = jQuery(rowId + ' .<?php echo $this->id; ?>_table_rate_zone').val();
						   				rowArr['service'] = jQuery(rowId + ' .<?php echo $this->id; ?>_table_rate_service').val();
						   				rowArr['class'] = jQuery(rowId + ' .<?php echo $this->id; ?>_table_rate_class').val();
						   				rowArr['basis'] = jQuery(rowId + ' .<?php echo $this->id; ?>_table_rate_basis').val();
						   				max = jQuery(rowId + ' .<?php echo $this->id; ?>_table_rate_max').val();
						   				if (max != '*') {
											rowArr['min'] = max;
										}
						   			}
						   		...
						   		function addTableRateRowHtml(isNew, rowArr) {
						   			...
						   			var html = '\
						   					<tr class="table_rate">\
												...
												<td>\
													<select name="<?php echo $this->id; ?>_table_rate_zone[' + size + ']" class="<?php echo $this->id; ?>_table_rate_zone">\
														' + generateSelectOptionsHtml(options['table_rate_zone'], rowArr['zone']) + '\
													</select>\
												</td>\
												<td>\
													<select name="<?php echo $this->id; ?>_table_rate_service[' + size + ']" class="<?php echo $this->id; ?>_table_rate_service">\
														' + generateSelectOptionsHtml(options['service'], rowArr['service']) + '\
													</select>\
												</td>\
												<td>\
													<select name="<?php echo $this->id; ?>_table_rate_class[' + size + ']" class="<?php echo $this->id; ?>_table_rate_class">\
														' + generateSelectOptionsHtml(options['shipping_class'], rowArr['class']) + '\
													</select>\
												</td>\
												<td>\
													<select name="<?php echo $this->id; ?>_table_rate_basis[' + size + ']" class="<?php echo $this->id; ?>_table_rate_basis">\
														' + generateSelectOptionsHtml(options['rate_basis'], rowArr['basis']) + '\
													</select>\
												</td>\
												<td>\
													<input type="text" name="<?php echo $this->id; ?>_table_rate_min[' + size + ']" value="' + rowArr['min'] + '" placeholder="0" size="4" class="<?php echo $this->id; ?>_table_rate_min" />\
												</td>\
												<td>\
													<input type="text" name="<?php echo $this->id; ?>_table_rate_max[' + size + ']" value="' + rowArr['max'] + '" placeholder="*" size="4" class="<?php echo $this->id; ?>_table_rate_max" />\
												</td>\
												<td>\
...
?>
				 ...
				 */
 				function process_zones() {
					...
					if ( isset( $_POST[ $this->id . '_zone_c_country'] ) ) $zone_c_country  = json_decode( stripslashes( json_encode( (array) $_POST[ $this->id . '_zone_c_country'] ) ), true );
					if ( isset( $_POST[ $this->id . '_zone_s_country'] ) ) $zone_s_country  = array_map( 'wc_clean', $_POST[ $this->id . '_zone_s_country'] );
					if ( isset( $_POST[ $this->id . '_zone_s_states'] ) ) $zone_s_states  = json_decode( stripslashes( json_encode( (array) $_POST[ $this->id . '_zone_s_states'] ) ), true );
					if ( isset( $_POST[ $this->id . '_zone_p_country'] ) ) $zone_p_country  = array_map( 'wc_clean', $_POST[ $this->id . '_zone_p_country'] );
					if ( isset( $_POST[ $this->id . '_zone_p_postcode'] ) ) $zone_p_postcode  = array_map( 'stripcslashes', $_POST[ $this->id . '_zone_p_postcode'] ); // wc_clean removes '\n'
					...
					for ( $i = 0; $i <= $key; $i++ ) {
						if ( isset( $zone_id[ $i ] ) 
							...
							switch ($zone_type[$i]) {
								case 'country':
									if (isset($zone_c_country[$i])) $country = $zone_c_country[$i];
									$values = '';
									break;
								case 'state':
				...
				function process_table_rates() {
					...
					if ( isset( $_POST[ $this->id . '_table_rate_cost'] ) )  $table_rate_cost  = array_map( 'wc_clean', $_POST[ $this->id . '_table_rate_cost'] );
					...
					for ( $i = 0; $i <= $key; $i++ ) {
						if ( isset( $table_rate_id[ $i ] ) 
							...
							if ( empty( $table_rate_min[ $i ] ) )
								$table_rate_min[ $i ] = '0';

							if ( empty( $table_rate_max[ $i ] ) )
								$table_rate_max[ $i ] = '*';
							
							if ( empty( $table_rate_cost[ $i ] ) )
								$table_rate_cost[ $i ] = '0';

							if ( empty( $table_rate_item_cost[ $i ] ) )
								$table_rate_item_cost[ $i ] = '0';

							if ( empty( $table_rate_weight_cost[ $i ] ) )
								$table_rate_weight_cost[ $i ] = '0';
				...
				/*
					...
					TODO: Really need to split this into component parts
				*/
				function get_available_zones($package) {
					...
					// Some country postcodes need processing to work with ranges
					$processed_destination_postcode = $this->get_processed_postcode($destination_postcode, $destination_country);

					// Remove spaces, used for some postcode ranges (CA)
					$spaceless_destination_postcode = str_replace(' ', '', $destination_postcode);

					// Remove 3 last characters and any spaces, used for some postcode ranges (UK, CA)
					if (strlen($destination_postcode) >= 3) {
						$trimmed_destination_postcode = trim(substr($destination_postcode, 0, -3));
					} else {
						$trimmed_destination_postcode = '';
					}
						...
						if ($zone['type'] == 'country') { // COUNTRY
							...
							if ($destination_country == $zone['country']) {
								...
								foreach ($postcodes as $postcode) {
									...
									if ($destination_postcode == $postcode) { // EXACT MATCH
									...
									} else if ($destination_country == 'CA') { // CANADAIAN POSTCODES ALSO NEED SPECIAL ATTENTION, EH

										if (preg_match("/^([ABCEGHJKLMNPRSTVXY]{1}\d{1}[A-Z]{1})$/i", $postcode, $matches)) { // CA FSA
										...
										} else if (preg_match("/^([ABCEGHJKLMNPRSTVXY]{1}\d{1}[A-Z]{1})-([ABCEGHJKLMNPRSTVXY]{1}\d{1}[A-Z]{1})$/i", $postcode, $matches)) { // CA FSA RANGE

											$min = $matches[1];
											$max = $matches[2];

											if (($trimmed_destination_postcode >= $min) && ($trimmed_destination_postcode <= $max)) {
												$found = true;
											}

										} else if (preg_match("/^([ABCEGHJKLMNPRSTVXY]{1}\d{1}[A-Z]{1}\d{1}[A-Z]{1}\d{1})-([ABCEGHJKLMNPRSTVXY]{1}\d{1}[A-Z]{1}\d{1}[A-Z]{1}\d{1})$/i", $postcode, $matches)) { // CA FULL RANGE
											
											$min = $matches[1];
											$max = $matches[2];

											if (($spaceless_destination_postcode >= $min) && ($spaceless_destination_postcode <= $max)) {
												$found = true;
											}
										...
										if ((is_numeric($processed_destination_postcode))
										 && ($processed_destination_postcode >= $min)
										 && ($processed_destination_postcode <= $max)) {
											$found = true;
										}
				...
				/*
					Process a postcode so that it works with ranges
				*/
				function get_processed_postcode($postcode, $country) {

					$processed_postcode = preg_replace("/[^0-9]/", "", $postcode);

					return $processed_postcode;
				}
				...
				*/
				function get_available_table_rates($package, $shipping_class, $calc_type) {
					...
					$weight = apply_filters( 'mhtrp_weight', $weight, $shipping_class);
					$total = apply_filters( 'mhtrp_subtotal', $total, $shipping_class);
					$item_count = apply_filters( 'mhtrp_item_count', $item_count, $shipping_class);
					...
					foreach ($this->table_rates as $table_rate):
						...
						// Trim and remove spaces from cost
						$table_rate['cost'] = str_replace(' ', '', $table_rate['cost']);

						// Calculate percentage cost
						if (!empty($table_rate['cost'])
							&& (substr($table_rate['cost'], -1) == '%')) {
							$percentage = substr($table_rate['cost'], 0, -1);
							if (!empty($percentage) && is_numeric($percentage)) {
								$table_rate['cost'] = $total * ($percentage/100);
							}
						}
				...
				*/
				...
				/*
					Calculate and return the handling fee
				*/
				function get_handling_fee() {

					// Trim and remove spaces from cost
					$handling_fee = str_replace(' ', '', $this->settings['handling_fee']);

					// Calculate percentage cost
					if (!empty($handling_fee)
						&& (substr($handling_fee, -1) == '%')) {
						$percentage = substr($handling_fee, 0, -1);
						if (!empty($percentage) && is_numeric($percentage)) {
							$cart_total = WC()->cart->cart_contents_total;
							$handling_fee = $cart_total * ($percentage/100);
						}
					}
				...
				*/
				public function calculate_shipping( $package ) {
					...
					foreach ($table_rates as $table_rate):
						...
						$cost = $table_rate['cost'] + $this->get_handling_fee();
	...
	function mh_wc_table_rate_plus_textdomain() {
		...
		load_plugin_textdomain( MHTRP_DOMAIN, false, $plugin_dir . '/languages' );
	}
